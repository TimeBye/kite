...
export function KubeconfigDownloadDialog({
  ...
  const expirationDate = useMemo(() => toExpirationDate(expiration), [expiration])
  ...
  const ttlError =
    custom && (customTTL < minTTL || customTTL > maxTTL)
      ? t('kubeconfigDownload.ttlError', 'Select an expiration time between 1 hour and 5 years from now.')
      : ''

  const toggleCluster = (uuid: string) => {
    setSelected((items) =>
      items.includes(uuid) ? items.filter((item) => item !== uuid) : [...items, uuid]
    )
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{t('kubeconfigDownload.title', 'Download Kubeconfig')}</DialogTitle>
          <DialogDescription>
            {t('kubeconfigDownload.description', 'Choose accessible clusters and an expiration time.')}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-5">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                onClick={() =>
                  setSelected(
                    selected.length === downloadableClusters.length
                      ? []
                      : downloadableClusters.map((cluster) => cluster.uuid)
                  )
                }
              >
                {selected.length === downloadableClusters.length
                  ? t('kubeconfigDownload.deselectAll', 'Deselect all')
                  : t('kubeconfigDownload.selectAll', 'Select all')}
              </Button>
            </div>
            <Input
              value={clusterSearch}
              onChange={(event) => setClusterSearch(event.target.value)}
              placeholder={t('kubeconfigDownload.searchClusters', 'Search clusters')}
            />
            <div className="max-h-44 space-y-2 overflow-y-auto rounded-md border p-3">
              {filteredClusters.map((cluster) => (
                <label key={cluster.uuid} className="flex cursor-pointer items-center gap-2 text-sm">
                  <Checkbox
              ...
              {filteredClusters.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  {t('kubeconfigDownload.noClustersFound', 'No matching clusters found.')}
                </p>
              )}
            </div>
          </div>
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {presets.map((preset) => (
                <Button
                  ...
                  size="sm"
                  variant={!custom && ttl === preset.value ? 'default' : 'outline'}
                  onClick={() => {
                    setCustom(false)
                    setTTL(preset.value)
                  }}
                >
                  {t(`kubeconfigDownload.presets.${preset.label}`, preset.label)}
                </Button>
              ))}
              <Button
                type="button"
            ...
            {custom && (
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
                {[
                  ['year', expiration.year, Array.from({ length: 6 }, (_, index) => String(new Date().getFullYear() + index))],
                  ['month', expiration.month, Array.from({ length: 12 }, (_, index) => pad(index + 1))],
                  [
                    'day',
                    expiration.day,
                    Array.from(
                  ...
                  ['hour', expiration.hour, Array.from({ length: 24 }, (_, index) => pad(index))],
                  ['minute', expiration.minute, Array.from({ length: 60 }, (_, index) => pad(index))],
                  ['second', expiration.second, Array.from({ length: 60 }, (_, index) => pad(index))],
                  ...
                  <div key={name as string} className="space-y-1">
                    ...
                    >
                      <SelectTrigger id={`kubeconfig-${name}`} className="w-full">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
            ...
            {effectiveTTL !== null && (
              <p className="text-xs text-muted-foreground">
                {t('kubeconfigDownload.expiresAt', 'Expires {{date}}', {
                  date: new Date(Date.now() + effectiveTTL * 1000).toLocaleString(),
                })}
              </p>
            )}
          ...
          <p className="border-t pt-3 text-xs text-muted-foreground">
            {t('kubeconfigDownload.securityNotice', 'This file contains a sensitive Bearer token. Store it securely and set file permissions to 0600.')}
          </p>
        </div>
        <DialogFooter>
          ...
          <Button disabled={downloading || selected.length === 0 || !!ttlError} onClick={handleDownload}>
            <Download className="mr-2 h-4 w-4" />
            {downloading ? t('kubeconfigDownload.downloading', 'Downloading...') : t('kubeconfigDownload.download', 'Download')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
