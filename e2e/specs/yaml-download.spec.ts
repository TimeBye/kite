...
test.describe("YAML download", () => {
    ...
    await page.route(
      ...
      async (route) => {
        ...
        await route.fulfill({
          ...
          headers: {
            "Content-Disposition": `attachment; filename="${controlPlaneNodeName}.yaml"`,
          },
          body: yamlContent,
        });
      },
  ...
  test("downloads a single namespace-scoped resource as raw YAML from the detail page", async ({
    ...
    await page.route(
      ...
      async (route) => {
        ...
        await route.fulfill({
          ...
          headers: {
            "Content-Disposition":
              'attachment; filename="kube-root-ca.crt.yaml"',
          },
          body: yamlContent,
        });
      },
    ...
    const download = page.waitForEvent("download");
    await page.getByRole("menuitem", { name: "Raw YAML" }).click();
    expect((await download).suggestedFilename()).toBe("kube-root-ca.crt.yaml");
  });
